...
class NotificationService:
    ...
    def format_notification(preference: NotificationPreference, context: Dict) -> Dict:
        ...
        deep_link = context.get('url') or event_routes.get(preference.event_type, '/app')
    ...
    @staticmethod
    def send_to_subscription(subscription: PushSubscription, notification: Dict, event_type: str, user: User) -> bool:
        """
        ...
        try:
            ...
            try:
                padded = info['private_key'] + '=' * ((4 - len(info['private_key']) % 4) % 4)
                private_key_bytes = base64.urlsafe_b64decode(padded)
                private_value = int.from_bytes(private_key_bytes, 'big')
                private_key_obj = ec.derive_private_key(private_value, ec.SECP256R1(), default_backend())
    ...
    def send_notification(
        ...
        for user in users_to_notify:
            ...
            for subscription in PushSubscription.objects.filter(user=user, is_active=True).iterator():
