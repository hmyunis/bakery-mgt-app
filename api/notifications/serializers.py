...
class PushSubscriptionCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if request and hasattr(request, "data"):
            ...
            logger.debug(
                f"[Subscription] Raw request.data keys: "
                f"{list(raw_data.keys()) if isinstance(raw_data, dict) else 'N/A'}"
            )
        ...
        if isinstance(raw_data, dict):
            ...
            if not p256dh or not auth:
                ...
                if isinstance(keys, dict):
                    logger.debug(
                        f"[Subscription] Checking nested keys: {list(keys.keys())}"
                    )
        ...
        logger.info(
            f"[Subscription] Validated subscription data - "
            f"endpoint: {attrs['endpoint'][:50]}..., has keys: ✓"
        )
        return attrs
    ...
    def create(self, validated_data):
        ...
        logger.info(
            f"[Subscription] Creating subscription for user {user.username} "
            f"with endpoint {endpoint[:50]}..."
        )
        ...
        logger.info(
            ...
            f"subscription {subscription.id} for user {user.username}"
        )
