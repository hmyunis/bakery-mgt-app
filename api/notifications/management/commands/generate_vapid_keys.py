...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Get the public key in uncompressed format (65 bytes: 0x04 + 32 bytes X + 32 bytes Y)
            ...
            self.stdout.write(f'VAPID_EMAIL=admin@bakery.com')
            self.stdout.write(self.style.SUCCESS('\n' + '=' * 60))
            self.stdout.write(self.style.WARNING('\n⚠️  IMPORTANT: Keep the private key secret!'))
            self.stdout.write(self.style.WARNING('   Never commit it to version control.'))
            self.stdout.write(self.style.SUCCESS('=' * 60 + '\n'))
