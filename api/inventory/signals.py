...
def update_inventory_on_purchase(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        send_notification(
            ...
            {
                "purchaser_name": instance.purchaser.username
                if instance.purchaser
                else "System",
        ...
        if instance.is_price_anomaly:
            percentage = (
                (
                    (instance.unit_cost - ingredient.average_cost_per_unit)
                    / ingredient.average_cost_per_unit
                    * 100
                )
                if ingredient.average_cost_per_unit > 0
                else 0
            )
            send_notification(
        ...
        if hasattr(instance, "_old_quantity"):
            ...
            if old_ingredient_id != instance.ingredient_id:
                ...
                new_ingredient.save(
                    update_fields=["current_stock", "last_purchased_price"]
                )
            else:
